...
Launching the shared shopping list starts the Deno application, a PostgreSQL server,
and a database migration process (Flyway).

## Starting and shutting down

The shared shopping list is used with Docker Compose.

- To start the shared shopping list, open up the terminal in the folder that
  contains the `docker-compose.yml` file and type `docker compose up`.
- To stop the shared shopping list, press `ctrl+C` (or similar) in the same terminal
...
The shared shopping list by default watches for changes in the Deno code and
...
When the shared shopping list is up and running, you can access the PostgreSQL
...
The shared shopping list comes also with simple
